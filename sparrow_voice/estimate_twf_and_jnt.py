...
class ETJ:
    ...
    def align_feature_vectors(self, odata, onpows, tdata, tnpows, pconf,
    ...
    def etj_main(self, org_yml, tar_yml, pair_yml, org_list, tar_list, pair_dir):

        # read speaker-dependent yml files
        oconf = SpeakerYML(org_yml)
        tconf = SpeakerYML(tar_yml)
        ...
        org_npows = read_feats(org_list, h5_dir, ext='npow')
        tar_mceps = read_feats(tar_list, h5_dir, ext='mcep')
        tar_npows = read_feats(tar_list, h5_dir, ext='npow')
        ...
        jmceps, twfs = self.align_feature_vectors(org_mceps,
                                            org_npows,
                                            tar_mceps,
                                            tar_npows,
                                            pconf,
        ...
        org_codeaps = read_feats(org_list, h5_dir, ext='codeap')
        tar_codeaps = read_feats(tar_list, h5_dir, ext='codeap')
        jcodeaps = []
        for i in range(len(org_codeaps)):
            # extract codeap joint feature vector
            jcodeap, _, _ = self.get_alignment(org_codeaps[i],
                                        org_npows[i],
                                        tar_codeaps[i],
                                        tar_npows[i],
                                        opow=oconf.power_threshold,
        ...
        jnt_dir = os.path.join(pair_dir, 'jnt')
